...
void Viewer::Motion(int x, int y)
{
	...
	if (m_Mouse_Event[0])
	{
		...
		else if (interationMode)
		{
			...
			vec3 mouse(0.0f, 0.0f, 1.0f);
			mouse.x = (1000.0f * x / 1024) - 500.0f+(tan(atan(((1000.0f * x / 1024) - 500.01f)/500))*(500-0.1));// ((2.0f * x / 1024) - 1) * 50 * 50 / 38;
			mouse.y = (1000.0f * (1 - y / 800)) - 500.0f+(tan(atan(((1000.0f * (1 - y / 800)) - 500.0f)/500))*(500-0.1)); //(230-y)/10+29;
			printf("%lf,%lf\n", mouse.x, mouse.y);
			for (int i = 0; i < 50; i++) {
				for (int j = 0; j < 50; j++) {
					if ((mouse - S_Simulator.cloth->nodes[i * 50 + j]->position).length() < 1) {
						S_Simulator.cloth->nodes[i * 50 + j]->position.x = mouse.x;
						S_Simulator.cloth->nodes[i * 50 + j]->position.y = mouse.y;
						glutPostRedisplay();
						return;
					}
				}
